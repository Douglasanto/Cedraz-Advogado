...
export function Header() {
  ...
  return (
    <>
      <header className="absolute w-full z-40 top-0">
            ...
            <div className="flex items-start gap-x-4 ">
              {navLinks.map((navLink) => (
                <Link
                  href={navLink.url}
                  key={navLink.id}
                  className="hover:text-primary text-white list-none font-semibold cursor-pointer transition-all duration-500 relative text-xl"
                >
                  {navLink.title}
                  <span className="absolute bottom-0 left-0 h-[0.125em] w-0 rounded-full bg-white duration-300 ease-in-out group-hover:w-full"></span>
                </Link>
              ))}
            </div>
          </div>
      ...
      <button
        ...
        aria-controls="menu"
        className={`z-[999] flex h-16 w-16 2xl:w-20 2xl:h-20 flex-col items-center justify-center rounded-full bg-[#a78a4e] ease-expo hover:scale-90 fixed left-6 top-6 sm:left-8 sm:top-8 transition-all duration-1000 group ${
      ...
      {menuVisible && (
        <>
          ...
          <div className="fixed z-50 flex h-svh w-screen pointer-events-none ease-expo">
            <div className="relative z-20 m-2 flex h-[300px] max-w-sm 2xl:max-w-xl 3xl:max-w-3xl flex-col justify-between rounded-md py-10 overflow-hidden bg-white pointer-events-auto w-[inherit] opacity-1 ">
